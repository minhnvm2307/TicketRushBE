...
- Recommendation-ready storage: `event_categories`, `event_tags`, `user_event_interactions`

## Database redesign for recommendations

Compared with the BRD baseline schema, this backend adds:

- `short_description`, `slug` on `events`
- `event_categories`
- `event_tags`
...
- weighting user actions such as view, hold, purchase
- recommendation scoring without introducing a second database

## Next extension points

- Replace `create_all()` with Alembic migrations in CI/CD
- Move queue state to Redis for higher throughput
- Add Kafka or RabbitMQ if checkout confirmation must become async
- Export custom business metrics, such as hold success rate and sell-through rate
